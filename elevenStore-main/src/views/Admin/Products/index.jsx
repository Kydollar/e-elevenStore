    ...
    customToolbarSelect: (selectedRows, displayData) => (
      <div className="flex gap-2 mr-6">
        {selectedRows.data.length <= 1 && (
          <Link to={`${displayData[selectedRows.data[0].index].data[0]}`} className="w-4 mx-1 transform hover:text-purple-500 hover:scale-110">
  ...
  return (
    <div className="relative w-full h-full">
      ...
      <ThemeProvider theme={getMuiTheme()}>
        <MUIDataTable title={"List of Products"} data={products.map((e) => [e.uuid, e, e.product_category.productCategoryName, e.price, e.stock])} columns={columns} options={options} />
      </ThemeProvider>
    </div>
  );
}
