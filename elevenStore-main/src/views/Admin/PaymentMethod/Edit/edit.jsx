...
export default function EditPaymentMethod() {
  ...
  const onSubmit = async (data) => {
    console.log(data);
    try {
      await axios.patch(`${process.env.REACT_APP_MY_API}/payment-methods/${uuid}`, data);
      Swal.fire({
        title: "Berhasil",
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="flex md:flex-row flex-col-reverse gap-4 items-center justify-center">
        <div className="flex flex-col w-full gap-y-4 flex-grow">
          <Controller control={control} name="name" render={({ field }) => <TextField {...field} label="Nama" error={!!errors.name} helperText={errors.name?.message} autoComplete="new-name" />} />
          <Controller
            control={control}
            name="paymentName"
            render={({ field }) => <TextField {...field} label="Nama Bank/Penyedia Layanan" error={!!errors.paymentName} helperText={errors.paymentName?.message} autoComplete="new-paymentName" />}
          />
          <Controller control={control} name="norek" render={({ field }) => <TextField {...field} label="Nomer Rekening" error={!!errors.norek} helperText={errors.norek?.message} autoComplete="new-norek" />} />
        </div>
      </div>
      <div className="mt-4">
