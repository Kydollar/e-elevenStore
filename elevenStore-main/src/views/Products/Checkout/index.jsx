...
export default function Checkout() {
	...
	const fetchShippingAddress = async () => {
		try {
			const response = await axios.get(
				`${process.env.REACT_APP_MY_API}/address-users/${userUuid}?primary=true`
			);
	...
	useEffect(() => {
		...
		const getAddress = async () => {
			try {
				const response = await axios.get(
					`${process.env.REACT_APP_MY_API}/address-users/${userUuid}`
				);
	...
	const handleBuatPesanan = (data) => {
		const cartItemCount = Object.values(data.cart).filter(
			...
			<div className="py-4 px-8 bg-white shadow-lg rounded-lg mt-10">
				<div className="flex flex-col gap-2 justify-center md:justify-start -mt-14 mb-5">
					<img
						...
						src="https://img.freepik.com/premium-vector/red-geolocation-icon_74669-526.jpg"
						alt=""
					/>
					...
					>
						{paymentMethods?.map((pm, idx) => (
							<FormControlLabel
								...
								classes={{
									root: `border ${
										selectedMethod === pm.uuid
											? "border-blue-500"
											: "hover:border-blue-900/50 hover:bg-blue-900/10 border-gray-300"
									}`,
									label: `px-4 py-2 ${
										selectedMethod === pm.uuid
											? "text-blue-500"
											: "hover:text-blue-900/50 text-gray-500"
									}`,
								}}
							/>
						))}
